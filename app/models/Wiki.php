...
class Wiki
{
    ...
    {
        try {
            $this->db->query("SELECT w.*, u.*, c.*, GROUP_CONCAT(t.name) AS tag_names FROM wikis w JOIN users u ON w.user_id = u.user_id JOIN categories c ON w.category_id = c.category_id JOIN wiki_tags wt ON wt.wiki_id = w.wiki_id JOIN tags t ON wt.tag_id = t.tag_id WHERE w.archiver = :archiver GROUP BY w.wiki_id; ");
    ...
    public function found_wiki($input)
    {
        ...
        JOIN
            wiki_tags wt ON wt.wiki_id = w.wiki_id
        JOIN
            tags t ON wt.tag_id = t.tag_id
        WHERE
            (w.title LIKE '%{$input}%' OR c.name LIKE '%{$input}%' OR t.name LIKE '%{$input}%') AND
            w.archiver = 1
        GROUP BY
            w.wiki_id;
    ...
    {
        $this->db->query('
    SELECT u.nom ,u.prenom , COUNT(w.user_id) as wikiCount
    FROM wikis w
