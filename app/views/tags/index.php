    ...
    <div class="container bg-white px-6 py-3 mx-auto md:flex">
        ...
        <div x-cloak :class="[isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full']" class="absolute bg-white md:bg-transparent inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out   md:mt-0 md:p-0 md:top-0 md:relative md:opacity-100 md:translate-x-0 md:flex md:items-center md:justify-between">
            <div class="flex flex-col px-2 -mx-4 md:flex-row md:mx-10 md:py-0">
                ...
                <a href="<?= URLROOT; ?>/tag/index" class="px-2.5 py-2 text-gray-700 transition-colors duration-300 transform rounded-lg dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 md:mx-2">TAGS</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const addTagForm = document.getElementById('addTagForm'); // Use the form ID here

        addTagForm.addEventListener('submit', function(event) {
            // Prevent the default form submission
           

            // Get the tag name input value
            const tagName = document.getElementById('categoryName').value;

            // Define a regex pattern for valid tag names
            const tagRegex = /^[a-zA-Z0-9_]+$/;

            // Check if the tag name matches the regex pattern
            if (!tagRegex.test(tagName)) {
                
                alert('Tag Name is invalid. It should contain only letters, numbers, and underscores.');
                return;
            }
